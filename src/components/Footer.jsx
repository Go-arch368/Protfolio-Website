...
export default function Footer() {
  return (
    <footer className="border-t border-[var(--color-border)] py-7 px-6 md:px-10 bg-[var(--color-bg)]">
      <div className="max-w-[1160px] mx-auto flex flex-col sm:flex-row items-center justify-between gap-5">
        <p className="font-mono text-[0.68rem] tracking-[0.06em] text-[var(--color-cream-3)]">
          © 2026 Gowtham T · Built with React + Tailwind
        </p>
        <nav className="flex items-center gap-6" aria-label="Social links">
          {[
            ...
            <a
              ...
              aria-label={s.label}
              className="text-[var(--color-cream-3)] hover:text-[var(--color-gold)] transition-colors duration-300"
            >
              {s.icon}
            </a>
          ))}
